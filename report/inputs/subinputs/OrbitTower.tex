...
\begin{align}
    ...
    \text{Mask}_{f} &= \text{Mask}_1 \cup \text{Mask}_2
\end{align}
\endgroup
...
\begin{align}
\begin{split}
    v_{linear} &= v_{base} \cdot f_{distance}(e_d) \omega_{angular} \\
            &= \omega_{base} + \alpha \cdot u_d + \beta \cdot p_{tower}
\end{split}
\end{align}
where:
\begin{align}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.25\textwidth]{Images/obstacle_avoidance.png}
...
\begin{multline}
(d_{Range2} > d_{clearance}) \land (d_{Range0} < d_{tower,\max})\\ 
\land \text{flag}_{almost\_avoided}
\end{multline}
